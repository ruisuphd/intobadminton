...
describe("parseCatalogFiltersFromSearchParams", () => {
  it("parses category and price band deep links", () => {
    const parsed = parseCatalogFiltersFromSearchParams(
      new URLSearchParams("cat=racket&brand=Yonex&price=under100")
    );
    expect(parsed.category).toBe("racket");
    expect(parsed.brand).toBe("Yonex");
    expect(parsed.priceBand).toBe("under100");
  });

  it("ignores unknown filter values", () => {
    expect(
      parseCatalogFiltersFromSearchParams(
        new URLSearchParams("cat=spaceship&price=free")
      )
    ).toEqual({});
  });
});
