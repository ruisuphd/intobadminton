/**
 * Golden-query regression guard for grip SKU commercial editorial exits.
 *
...
export type CommercialGripBaselineQuery = {
  id: string;
  productId: string;
  expectHref: string;
...
export type CommercialGripBaselineIssue = {
  id: string;
  message: string;
  note?: string;
};

export type CommercialGripBaselineResult = {
  ok: boolean;
  issues: CommercialGripBaselineIssue[];
  checked: number;
};

export function validateCommercialGripBaselineFile(
  data: unknown
): { ok: true; file: CommercialGripBaselineFile } | { ok: false; message: string } {
  ...
  const queries: CommercialGripBaselineQuery[] = [];
...
export function evaluateCommercialGripBaselineQuery(
  spec: CommercialGripBaselineQuery,
  product: ProductRecord | undefined
): CommercialGripBaselineIssue | null {
  if (!product) {
    return {
      id: spec.id,
...
export function evaluateCommercialGripBaseline(
  file: CommercialGripBaselineFile,
...
export function formatCommercialGripBaselineIssues(
  result: CommercialGripBaselineResult
): string {
  if (result.ok) {
    return `[commercial-grip-baseline] ${result.checked} golden queries passed`;
  }
  const lines = result.issues.map((issue) => {
    const note = issue.note ? ` (${issue.note})` : "";
    return `  • ${issue.id}: ${issue.message}${note}`;
  });
  return `[commercial-grip-baseline] ${result.issues.length} failure(s):\n${lines.join("\n")}`;
}
