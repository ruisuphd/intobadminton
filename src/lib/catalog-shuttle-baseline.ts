/**
 * Golden-query regression guard for shuttle SKU catalog editorial exits.
 *
 * Committed expectations live in `docs/baselines/catalog-shuttle-queries.json`.
 */
...
export type CatalogShuttleBaselineQuery = {
  id: string;
  productId: string;
  expectHref: string;
...
export type CatalogShuttleBaselineFile = {
  version: number;
  updated?: string;
  coverage?: BaselineE2eCoverage;
  queries: CatalogShuttleBaselineQuery[];
};

export type CatalogShuttleBaselineIssue = {
  id: string;
  message: string;
  note?: string;
};

export type CatalogShuttleBaselineResult = {
  ok: boolean;
  issues: CatalogShuttleBaselineIssue[];
  checked: number;
};

export function validateCatalogShuttleBaselineFile(
  data: unknown
): { ok: true; file: CatalogShuttleBaselineFile } | { ok: false; message: string } {
  ...
  const queries: CatalogShuttleBaselineQuery[] = [];
...
export function evaluateCatalogShuttleBaselineQuery(
  spec: CatalogShuttleBaselineQuery,
  product: ProductRecord | undefined
): CatalogShuttleBaselineIssue | null {
  if (!product) {
    return {
      id: spec.id,
...
export function evaluateCatalogShuttleBaseline(
  file: CatalogShuttleBaselineFile,
  lookup: (productId: string) => ProductRecord | undefined
): CatalogShuttleBaselineResult {
  const issues: CatalogShuttleBaselineIssue[] = [];
  ...
  for (const spec of file.queries) {
    const issue = evaluateCatalogShuttleBaselineQuery(spec, lookup(spec.productId));
...
export function formatCatalogShuttleBaselineIssues(
  result: CatalogShuttleBaselineResult
): string {
  if (result.ok) {
    return `[catalog-shuttle-baseline] ${result.checked} golden queries passed`;
  }
  const lines = result.issues.map((issue) => {
    const note = issue.note ? ` (${issue.note})` : "";
    return `  • ${issue.id}: ${issue.message}${note}`;
  });
  return `[catalog-shuttle-baseline] ${result.issues.length} failure(s):\n${lines.join("\n")}`;
}
