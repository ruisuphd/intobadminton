/**
 * Golden-query regression guard for racket SKU catalog editorial exits.
 *
 * Committed expectations live in `docs/baselines/catalog-racket-queries.json`.
 */
...
export type CatalogRacketBaselineQuery = {
  id: string;
  productId: string;
  expectHref: string;
...
export type CatalogRacketBaselineFile = {
  version: number;
  updated?: string;
  queries: CatalogRacketBaselineQuery[];
};

export type CatalogRacketBaselineIssue = {
  id: string;
  message: string;
  note?: string;
};

export type CatalogRacketBaselineResult = {
  ok: boolean;
  issues: CatalogRacketBaselineIssue[];
  checked: number;
};

export function validateCatalogRacketBaselineFile(
  data: unknown
): { ok: true; file: CatalogRacketBaselineFile } | { ok: false; message: string } {
  ...
  const queries: CatalogRacketBaselineQuery[] = [];
...
export function evaluateCatalogRacketBaselineQuery(
  spec: CatalogRacketBaselineQuery,
  product: ProductRecord | undefined
): CatalogRacketBaselineIssue | null {
  if (!product) {
    return {
      id: spec.id,
...
export function evaluateCatalogRacketBaseline(
  file: CatalogRacketBaselineFile,
  lookup: (productId: string) => ProductRecord | undefined
): CatalogRacketBaselineResult {
  const issues: CatalogRacketBaselineIssue[] = [];

  for (const spec of file.queries) {
    const issue = evaluateCatalogRacketBaselineQuery(spec, lookup(spec.productId));
...
export function formatCatalogRacketBaselineIssues(
  result: CatalogRacketBaselineResult
): string {
  if (result.ok) {
    return `[catalog-racket-baseline] ${result.checked} golden queries passed`;
  }
  const lines = result.issues.map((issue) => {
    const note = issue.note ? ` (${issue.note})` : "";
    return `  • ${issue.id}: ${issue.message}${note}`;
  });
  return `[catalog-racket-baseline] ${result.issues.length} failure(s):\n${lines.join("\n")}`;
}
