...
| 4 | Original photography / video | AdSense + experience signal | ⏳ Partial — catalogue-backed image CI on 17/17 commercial landings; tier-4 backfill + video deferred |
...
**Sprint 121 addendum:** Compliance trust-path e2e completion + retention funnel CrUX expansion — ✅ trust-path e2e 9→13; `minE2eGuards: 13` on trust-path baseline; `/privacy/` + `/terms/` + `/cookies/` + `/privacy-choices/` in trust-path e2e; `/cookies/` + `/privacy-choices/` + `/saved/` + `/compare/` in `crux-template.csv` + offline recovery; `/cookies/` in lighthouse baseline (noindex saved/compare/privacy-choices exempt).
...
- **Best-of image coverage CI** — `lint:best-image-coverage` guards commercial `/best/*` photography
...
### Sprint 76 — This branch

- **Catalogue image backfill** — Voltric 8DG (e78.us) + ASICS Blast FF 3 (asics.com CDN)
- **Image CI full coverage** — all 17 commercial `/best/*` landings guarded with waiver-adjusted thresholds
...
### Sprint 103 — This branch (`cursor/web-app-improvement-plan-983f`)

- **L69 PDP golden profile** — completes six-SKU string editorial corpus on `/product/[id]/`
- **Three-way string parity** — PDP ↔ catalog ↔ commercial baselines must agree per productId
