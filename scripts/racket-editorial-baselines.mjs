#!/usr/bin/env node
/**
 * Run all racket editorial regression guards in one operator command:
 * catalog exits, commercial exits, and three-way PDP parity.
 *
 * Usage:
 *   node scripts/racket-editorial-baselines.mjs
 *   node scripts/racket-editorial-baselines.mjs --help
 */
import { execFileSync } from "node:child_process";
import { resolve, dirname } from "node:path";
import { fileURLToPath } from "node:url";

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");

const GUARDS = [
  {
    name: "catalog racket editorial exits",
    script: "scripts/catalog-racket-baseline.mjs",
  },
  {
    name: "commercial racket editorial exits",
    script: "scripts/commercial-racket-baseline.mjs",
  },
  {
    name: "racket editorial three-way parity",
    script: "scripts/racket-editorial-parity.mjs",
  },
];

function usage() {
  console.log(`Usage:
  node scripts/racket-editorial-baselines.mjs   Run racket editorial golden-profile guards
`);
}

if (process.argv.includes("--help") || process.argv.includes("-h")) {
  usage();
  process.exit(0);
}

for (const guard of GUARDS) {
  console.log(`\n[racket-editorial-baselines] → ${guard.name}`);
  execFileSync("node", [guard.script], { cwd: ROOT, stdio: "inherit" });
}

console.log("\n[racket-editorial-baselines] all guards passed");
