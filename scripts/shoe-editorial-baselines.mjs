#!/usr/bin/env node
/**
 * Run all shoe editorial regression guards in one operator command:
 * catalog exits, commercial exits, and three-way PDP parity.
 *
 * Usage:
 *   node scripts/shoe-editorial-baselines.mjs
 *   node scripts/shoe-editorial-baselines.mjs --help
 */
import { execFileSync } from "node:child_process";
import { resolve, dirname } from "node:path";
import { fileURLToPath } from "node:url";

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");

const GUARDS = [
  {
    name: "catalog shoe editorial exits",
    script: "scripts/catalog-shoe-baseline.mjs",
  },
  {
    name: "commercial shoe editorial exits",
    script: "scripts/commercial-shoe-baseline.mjs",
  },
  {
    name: "shoe editorial three-way parity",
    script: "scripts/shoe-editorial-parity.mjs",
  },
];

function usage() {
  console.log(`Usage:
  node scripts/shoe-editorial-baselines.mjs   Run shoe editorial golden-profile guards
`);
}

if (process.argv.includes("--help") || process.argv.includes("-h")) {
  usage();
  process.exit(0);
}

for (const guard of GUARDS) {
  console.log(`\n[shoe-editorial-baselines] → ${guard.name}`);
  execFileSync("node", [guard.script], { cwd: ROOT, stdio: "inherit" });
}

console.log("\n[shoe-editorial-baselines] all guards passed");
