...
const PRODUCT_LABELS: Record<string, RegExp> = {
  ...
  "vic-carbonsonic-max-shuttle": /Carbonsonic MAX/i,
  "rsl-supreme-shuttle": /Supreme/i,
  "rsl-aero-u-shuttle": /Aero U/i,
  "rsl-aero-classic-tourney-shuttle": /Classic Tourney/i,
  "rsl-no4-plus-shuttle": /No\.?4 Plus/i,
};
...
for (const spec of e2eQueries()) {
  test(`catalog shuttle editorial exit: ${spec.id}`, async ({ page }) => {
    const label = PRODUCT_LABELS[spec.productId];
    expect(label).toBeDefined();

    await page.goto("/catalog/?cat=shuttle");
